...
public class ControllerToolMethod {
    ...
    /*---------------------------------------    向前端发送错误响应   ------------------------------------------*/
    private static void sendError(HttpServletResponse response, int code, String message) throws IOException, IOException {
    ...
    public static void fetchException(HttpServletRequest request, HttpServletResponse response, Exception e, String errorMsg) {
        try {
            if (e instanceof NumberFormatException) {
                sendError(response, Constants.RESPONSE_CODE_BAD_REQUEST, "参数格式有误");
            } else if (e instanceof NullPointerException) {
                sendError(response, Constants.RESPONSE_CODE_BAD_REQUEST, "空指针异常");
            } else if (e instanceof SQLException) {
                ControllerToolMethod.fetchException(request, response, e, "数据库操作异常: " + errorMsg);
            } else {
